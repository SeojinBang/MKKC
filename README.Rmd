...
Centering and scaling of kernel matrices in multi-view learning allow multiple views comparable with each other. Hence, we recommand to standardize the kernel matrices before combining them. Each kernel matrix is centered by $\mathbf{K} \leftarrow \mathbf{K}\text{ -- }\mathbf{J}_n \mathbf{K}\text{ -- }\mathbf{K}\mathbf{J}_n + \mathbf{J}_n \mathbf{K}\mathbf{J}_n$ and scaled by $\mathbf{K} \leftarrow n\mathbf{K} / tr(\mathbf{K})$ where $\mathbf{J}_n = \mathbf{1}_n\mathbf{1}_n^T/n$ and $n$ is the number of samples.
